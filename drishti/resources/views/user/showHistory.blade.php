<x-app-layout>
    ...
    <div class="container m-3">
        ...
        <img src="{{ asset('storage/'.$image->img) }}" id="hiddenImg" hidden>
    ...
    <div class="container mt-5 pt-5">
        ...
        <div class="row">
            @foreach($colors as $key => $color)
                <h6>
                    {{$key+1}} : <div style="height: 10px;width: 10px;background:rgb({{$color->colorcode}});display: inline-block;"></div>
                    - {{ $color->colorname }} ({{ $color->colorcode }}) 
                    <a href="#" type="button" onclick="draw({{$key+1}},'{{$color->colorname}}',{{$color->colorpointx}},{{$color->colorpointy}});">
                        Load
                    </a>
                </h6>
            @endforeach
    ...
    <script>
        ...
        loc = new Image();
        loc.src = "{{ asset('assets/images/map-marker.png') }}"

        function draw(k, c, x, y) {
            context = document.getElementById('canvas').getContext('2d');
            context.drawImage(loc, x - 50, y - 150, 100, 150);

            var markerText = k + ' - ' + c;

            // Draw a simple box so you can see the position
            var textMeasurements = context.measureText(k);
            ...
            context.fillText(markerText, x-5, y-100);
        }
